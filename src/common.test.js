...
describe('Common API', () => {
  ...
  it('hashs', async () => {
    ...
    for (const hash of hashes) {
      assert.equal(await ecc[hash[0]](''), hash[1])
      assert.equal(await ecc[hash[0]](Buffer.from('')), hash[1])
    }
  })
  ...
  it('signatures', async () => {
    ...
    for (const sig of sigs) {
      assert(await ecc.verify(sig, data, pubkey), 'verify data')
      assert(await ecc.verifyHash(sig, dataSha256, pubkey), 'verify hash')
      assert.equal(pubkey, await ecc.recover(sig, data), 'recover from data')
      assert.equal(pubkey, await ecc.recoverHash(sig, dataSha256), 'recover from hash')
    }
  })
})
