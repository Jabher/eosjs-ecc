...
export function deterministicGenerateK({n}, hash: Buffer, d, checkSig, nonce) {
    ...
    while ((T.signum() <= 0) || (T.compareTo(n) >= 0) || !checkSig(T)) {
...
export function sign(curve, hash, d, nonce) {
    ...
    let r
    let s
    const k = deterministicGenerateK(curve, hash, d, k => {
